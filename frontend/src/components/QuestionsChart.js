...
export default class QuestionsChart extends React.Component {
    ...
    componentDidMount() {
        axios.post('/getPopularQuestions').then(response => {
            console.log(response.data)

            this.setState({series: [{
                    data: [
                        {y:response.data[0].freq, color: '#43a9f7', label: response.data[0].date.substring(0,10)},
                        {y:response.data[1].freq, color: '#91bcff', label: response.data[1].date.substring(0,10)},
                        {y:response.data[2].freq, color: '#ff822e', label: response.data[2].date.substring(0,10)}
                    ]
                }]})
    ...
    render() {
        return(
            <div  style={{textAlign:'center' , marginTop:"20px", marginBottom:"20px", display:"inline-block", border:"1px solid black"}}>
                <p style={{marginTop:"28px", display:"inline-block", border:"1px solid black"}} className="white-banner">Popular dates</p>
